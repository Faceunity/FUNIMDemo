    ...
    [[NIMSDK sharedSDK].chatManager queryMessageReceiptDetail:self.message completion:^(NSError * _Nullable error, NIMTeamMessageReceiptDetail * _Nullable detail) {
        ...
        if (!error)
        {
            for (NSString *userId in detail.readUserIds)
            {
                ...
                if (member)
                {
                    [weakSelf.readMembers addObject:[[NIMTeamCardMemberItem alloc] initWithMember:member]];
                }
                else
                {
                    //群成员异常，可能是被踢了
                    [weakSelf.readMembers addObject:[[NIMUserCardMemberItem alloc] initWithUserId:userId]];
                }
            ...
            for (NSString *userId in detail.unreadUserIds)
            {
                ...
                if (member)
                {
                    [weakSelf.unreadMembers addObject:[[NIMTeamCardMemberItem alloc] initWithMember:member]];
                }
                else
                {
                    [weakSelf.unreadMembers addObject:[[NIMUserCardMemberItem alloc] initWithUserId:userId]];
                }
            }
